...
class DiT(nnx.Module):
    def __init__(self, embed_dim, num_heads, img_size, patch_size, in_ch, depth, rngs):
...
def patchify(x, patch_size):
    ...
    x = x.reshape(B, num_patch_h, patch_size, num_patch_w, patch_size, C)
    x = x.transpose(0, 1, 3, 2, 4, 5)
    return x.reshape(B, num_patch_h*num_patch_w, patch_size*patch_size*C)
...
rngs = nnx.Rngs(0)
model = DiT(EMBED_DIM, NUM_HEADS, IMG_SIZE, PATCH_SIZE, IN_CH, DEPTH, rngs=rngs)
optimizer = nnx.Optimizer(model, optax.adamw(learning_rate=1e-4, weight_decay=1e-4), wrt=nnx.Param)

x_dummy = jax.random.uniform(rngs(), (BATCH_SIZE, IMG_SIZE, IMG_SIZE, IN_CH), minval=-1.0, maxval=1.0)
...
for i in range(10):
    ...
    loss = train_step(model, optimizer, x_dummy, alpha_bar, PATCH_SIZE, subkey)
    print(f"Step: {i} | Token MSE loss: {loss:.4f}")