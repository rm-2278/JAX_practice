...
class CausalBlockAttention(nnx.Module):
...
class TransformerBlock(nnx.Module):
    def __init__(self, embed_dim, num_heads, rngs):
        ...
        self.attn = CausalBlockAttention(embed_dim, num_heads, rngs=rngs)
        self.ln2 = nnx.LayerNorm(embed_dim, rngs=rngs)
        self.mlp = MLP(embed_dim, rngs=rngs)
        
    def __call__(self, x):
        x = x + self.attn(self.ln1(x))  # Prenorm, don't normalize the residual part. if self.ln(attn(x)), x will dominate and also input to attn will be large
...
def train_step(model, optimizer, tokens):
    def loss_fn(model):
        ...
        loss = optax.softmax_cross_entropy_with_integer_labels(shift_logits, shift_tokens).mean()
        return loss
