...
class CausalSelfAttention(nnx.Module):
    def __init__(self, embed_dim, num_heads, rngs):
        ...
        assert embed_dim == self.head_dim * num_heads, "embed_dim must be divisible by num_heads"
    ...
    def __call__(self, x):
        ...
        score = jnp.matmul(q, k.transpose(0, 1, 3, 2)) / jnp.sqrt(self.head_dim)
        mask = jnp.tril(jnp.ones((T, T)))
        score = jnp.where(mask==0, -1e9, score)
        weight = nnx.softmax(score)
        out = jnp.matmul(weight, v).transpose(0, 2, 1, 3).reshape(B, T, C)
...
class MiniGPT(nnx.Module):
    ...
    def __call__(self, tokens):
        return self.lm_head(self.ln(self.block(self.token_emb(tokens))))
