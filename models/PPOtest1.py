...
def train_step(model, optimizer, batch):
    loss, grads = nnx.value_and_grad(ppo_loss)(model, batch['states'], batch['actions'], batch['old_log_probs'], batch['advantages'], batch['returns'])
    optimizer.update(model, grads)
    return loss

env = gym.make('Pendulum-v1')
model = PPO(obs_dim=3, act_dim=1, rngs=nnx.Rngs(42))
...
for epoch in range(1000):
    ...
    for _ in range(1000):
        ...
        states.append(state); actions.append(action); log_probs.append(log_prob); next_states.append(next_state)
