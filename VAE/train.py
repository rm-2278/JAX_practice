...
def loss_fn(key, model, params, batch):
    ...
    recon_loss = -jnp.sum(imgs * jnp.log(recon_imgs) + (1 - imgs)  * jnp.log(1 - recon_imgs), axis=1) # Binary cross entropy loss
...
class TrainerModule:
    ...
    def create_functions(self):
        def train_step(key, batch, state):
            loss, grads = jax.value_and_grad(loss_fn, argnums=1)(key, self.model, state.params, batch)
    ...
    def train_epoch(self, epoch):
        ...
        for batch in self.train_dataloader:
            self.state, loss = self.train_step(batch, self.state)
            losses.append(loss)
    ...
    def eval_model(self, val_dataloader):
        ...
        for batch in val_dataloader:
            loss = self.eval_step(batch, self.state)
            losses.append(loss)
